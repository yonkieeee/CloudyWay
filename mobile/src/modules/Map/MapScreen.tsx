...
const AuthScreen: React.FC = () => {
  ...
  const fetchMarkers = async (): Promise<Marker[]> => {
    try {
      const dbUrl = `http://52.59.184.132:5001/places/getAllPlaces`;
      ...
      await fetch("https://52.59.184.132:5001/places/addPlace", {
        method: "POST",
        headers: {
  ...
  const fetchSearchResults = async (query: string) => {
    ...
    try {
      const response = await fetch(`http://52.59.184.132:5001/places/fuzzySearch?query=${query}`);
  ...
  const handleAddNewPlace = async () => {
    ...
    try {
      const discoverUrl = `http://52.59.184.132:5000/locations/discover?query=${encodeURIComponent(newPlaceName)}`;
  ...
  const handleSelectDiscoveredPlace = async (place: any) => {
    ...
    try {
      const addResponse = await fetch('http://52.59.184.132:5001/places/addPlace', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
