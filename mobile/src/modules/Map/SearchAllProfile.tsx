...
const FriendsProfile: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const allUsersRes = await axios.get("http://3.121.196.125:5002/users/getAllUsers");
                const allUids: string[] = allUsersRes.data;

                const followingRes = await axios.get(
                    `http://3.121.196.125:5002/users/getFollowing/${uid}`
                );
    ...
    const toggleStatus = async (friend: Friend) => {
        ...
        if (status === "follow") {
            Alert.alert("Unfollow", `Are you sure you want to unfollow ${friend.name}?`, [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await axios.delete(
                                `http://3.121.196.125:5002/users/deleteFollow/${currentUid}/${uid}`
                            );
            ...
            try {
                await axios.post(
                    `http://3.121.196.125:5002/users/createFollow/${currentUid}/${uid}`
                );
                setFriendsState((prevState) =>
                    prevState.map((f) =>
